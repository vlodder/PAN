    ...
    : BaseViewModel(dialogService, navigationService)
{
    ...
    [ObservableProperty]
    private string selectedVille = "Tous";
    ...
    [ObservableProperty]
    private ObservableCollection<string> mouvements =
    [
        "Tous",
        "Oui",
        "Non"
    ...
    private async Task InitFiltersAsync()
    {
        try
        {
            ...
            foreach (var ville in villesList)
            {
                Villes.Add(ville);
            }
    ...
    private async Task LoadMoreAsync()
    {
        ...
        try
        {
            ...
            var items = await _evenementService.SearchPagedAsync(
                SearchText,
                SelectedVille == "Tous" ? null : SelectedVille,
                SelectedType?.Id,
                estMouvant,
                _currentSkip,
                PageSize);
    ...
    private async Task OpenDetailAsync(EvenementListItem item)
    {
        ...
        await Shell.Current.GoToAsync(
            $"EventDetailPage?idEvenement={item.IdEvenement}");
    }
